...
def calculate_similar(imageVal: torch.tensor, overall: torch.tensor, itrn_id: int, classes, root_dir, debug: bool = False):
	...
	if minVal < 0.8:
		...
		globalMin = f'./detected_faces/face_{itrn_id}.jpg'

		return list(classes.keys())[argMin], globalMin, f"{root_dir}/{list(classes.keys())[argMin]}.png"
...
def run_all(imgStr: str, root_dir: str, show_image: bool = False, debug: bool = False):
	...
	for i in range(len(overall)):
		# print(f"Iteration {i}")
		image = calculate_similar(overall[i], references, i, classes, root_dir, debug=debug)
		if image:
			images.append(image)
...
def get_contact(id):
	with open("mycontacts.json", "r") as f:
		mycontacts = json.load(f)
	return mycontacts[id]
